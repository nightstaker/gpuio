...
static int context_init_devices(gpuio_context_t ctx) {
    /* Try to initialize NVIDIA first */
    if (nvidia_ops.device_init && nvidia_ops.device_init(ctx, 0) == 0) {
        LOG(ctx, GPUIO_LOG_INFO, "Initialized NVIDIA GPU support");
        current_vendor_ops = &nvidia_ops;
        return device_detect_all(ctx);
    }
    
    /* Try AMD next */
    if (amd_ops.device_init && amd_ops.device_init(ctx, 0) == 0) {
        LOG(ctx, GPUIO_LOG_INFO, "Initialized AMD GPU support");
        current_vendor_ops = &amd_ops;
        return device_detect_all(ctx);
    }
    
    /* No GPUs found - use stub mode for testing */
    LOG(ctx, GPUIO_LOG_WARN, "No GPUs detected, running in stub mode");
    ctx->num_devices = 0;
    ctx->devices = NULL;
    return 0;
}
...
gpuio_error_t gpuio_init(gpuio_context_t* ctx_ptr, const gpuio_config_t* config) {
    ...
    int ret = context_init_devices(ctx);
    if (ret != 0) {
        LOG(ctx, GPUIO_LOG_ERROR, "Failed to initialize devices: %d", ret);
        free(ctx);
        pthread_mutex_unlock(&global_lock);
    ...
    *ctx_ptr = ctx;
    ...
    LOG(ctx, GPUIO_LOG_INFO, "gpuio initialized (version %s)",
...
gpuio_error_t gpuio_finalize(gpuio_context_t ctx) {
    ...
    LOG(ctx, GPUIO_LOG_INFO, "Finalizing gpuio context");
    ...
    while (region) {
        memory_region_internal_t* next = region->next;
...
int device_detect_all(gpuio_context_t ctx) {
    if (!current_vendor_ops || !current_vendor_ops->device_get_info) {
        return -1;
    }
    
    /* Try to detect up to 16 GPUs */
    ctx->devices = calloc(16, sizeof(device_info_internal_t));
    if (!ctx->devices) {
        return -1;
    }
    ...
    LOG(ctx, GPUIO_LOG_INFO, "Detected %d GPU(s)", ctx->num_devices);
    return 0;
}
...
gpuio_error_t gpuio_get_device_info(gpuio_context_t ctx, int device_id, 
    ...
    device_info_internal_t* internal = &ctx->devices[device_id];
...
gpuio_error_t gpuio_set_device(gpuio_context_t ctx, int device_id) {
    ...
    if (current_vendor_ops && current_vendor_ops->device_set_current) {
        int ret = current_vendor_ops->device_set_current(ctx, device_id);
        if (ret != 0) {
            return GPUIO_ERROR_GENERAL;
        }
    }
