/**
 * @file vendor_amd.c
 * @brief AMD ROCm/HIP vendor-specific implementation (stub)
 * @version 1.0.0
 */
...
/* Stub implementation for when ROCm is not available */
/* In full implementation, this would include <hip/hip_runtime.h> */
...
int amd_unregister_memory(gpuio_context_t ctx, memory_region_internal_t* region) {
    (void)ctx;
    (void)region;
    return 0;
}

int amd_stream_create(gpuio_context_t ctx, stream_internal_t* stream,
...
int amd_stream_destroy(gpuio_context_t ctx, stream_internal_t* stream) {
    (void)ctx;
    (void)stream;
    return 0;
}

int amd_stream_synchronize(gpuio_context_t ctx, stream_internal_t* stream) {
    (void)ctx;
    (void)stream;
    return 0;
}

int amd_stream_query(gpuio_context_t ctx, stream_internal_t* stream, bool* idle) {
    (void)ctx;
    (void)stream;
    *idle = true;
    return 0;
...
/* Current vendor ops pointer */
vendor_ops_t* current_vendor_ops = NULL;
